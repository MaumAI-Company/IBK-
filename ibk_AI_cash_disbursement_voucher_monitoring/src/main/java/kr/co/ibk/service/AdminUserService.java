...
public class AdminUserService extends _BaseService {
    ...
    public Boolean confirmMemSno(MemberForm form) {
        int result = memberRepository.confirmMemSno(form);
        return result > 0;
    }
    ...
    public HashMap<String, Object> addUser(MemberInfo params) {
        ...
        // 발신여부 Y시 다른 유저 발신여부 N 처리
        if (params.getSndYn().equals("Y")) {
            adminMemberRepository.updateSndN(params.getMemSeq());
        }
