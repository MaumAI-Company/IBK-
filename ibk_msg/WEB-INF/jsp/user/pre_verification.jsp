        ...
        <div id="contents">
            ...
            <div class="content">
                ...
                <div class="srchArea">
					...
					<input type="hidden" id="h_searchFit" name="h_searchFit" value="${params.searchFit}"/>  	<!-- 일치여부 → 통과여부 -->
                            ...
                            <tr>
                                ...
                                <td>
                                   <select id="searchChannel" class="select">
                                        <option value="전체" <c:if test="${empty params.searchChannel or params.searchChannel eq '전체'}">selected="selected"</c:if>>전체</option>
                                        <option value="crm" <c:if test="${params.searchChannel eq 'crm'}">selected="selected"</c:if>>CRM</option>
                                        <option value="ibk" <c:if test="${params.searchChannel eq 'ibk'}">selected="selected"</c:if>>IBK메시지센터</option>
                             ...
                             <tr>
                                ...
                                <td>
                                   <select id="searchFit" class="select">
                                        ...
                                        <option value="fit" <c:if test="${params.searchFit eq 'fit'}">selected="selected"</c:if>>통과</option>
                                        <option value="notfit" <c:if test="${params.searchFit eq 'notfit'}">selected="selected"</c:if>>불통과</option>
                                        <%--<option value="test">테스트</option>--%>
                                    </select>
                                </td>
                                <th>정렬조건<br>(검증일시)</th>
                                ...
                                <td>
                                   <select id="searchType" class="select">
                                        ...
                                        <option value="em" <c:if test="${params.searchType eq 'em'}">selected="selected"</c:if>>직원</option>
                    ...
                    <table summary="연번, 실행 일시, 구분, 확인 결과, 전행고객번호, 상세보기로 구성">
                        ...
                        <thead>
                            <tr>
								...
								<!-- 2023-01-27 상세보기의 내용 추가(add) start -->
								<!-- <th scope="col">내용</th> -->
								<!-- 2023-01-27 상세보기의 내용 추가(add) end --> 
                            </tr>
                        </thead>
                        <tbody>
                        	<c:choose>
                        		...
                        		<c:when test="${!empty msgList }">
                        			<c:forEach var="list" varStatus="status" items="${msgList}" >
			                            <tr>
			                                ...
			                                <td>
			                                	<c:choose>
			                                		<c:when test="${list.channel eq 'ibk'}">IBK메시지센터</c:when>
			                                ...
			                                <td>
			                                	<c:choose>
			                                		<c:when test="${list.isMatch eq '0'}"><span class="ft_point_pink">불통과</span></c:when>
			                                		<c:when test="${list.isMatch eq '1'}"><span class="">통과</span></c:when>
                 ...
                 <div id="msgPagingArea" class="pagingArea">
					...
					<div class="paging">
						<c:if test="${params.paginationInfo.hasPreviousPage}">
							<a href="javascript:movePage('/user/pre_verification', '${params.makeQueryString(1)}');" class="btn_paging first">
	                            <span class="hide">처음 페이지</span>
	                            <em class="fas fa-angle-double-left"></em>
	                        </a>
							<a href="javascript:movePage('/user/pre_verification', '${params.makeQueryString(params.paginationInfo.firstPage-1)}');" class="btn_paging prev">
                       	...
                       	<c:forEach begin="${params.paginationInfo.firstPage}" end="${params.paginationInfo.lastPage}" var="pageNo">
							...
							<a href="javascript:movePage('/user/pre_verification', '${params.makeQueryString(pageNo)}');"  class="<c:out value="${params.currentPageNo == pageNo ? 'current' : ''}"/>">${pageNo}</a>
                       	</c:forEach>
						<c:if test="${params.paginationInfo.hasNextPage}">
							<a href="javascript:movePage('/user/pre_verification', '${params.makeQueryString(params.paginationInfo.lastPage+1)}');" class="btn_paging next">
	                            <span class="hide">다음 페이지</span>
	                            <em class="fas fa-angle-right"></em>
	                        </a>
							<a href="javascript:movePage('/user/pre_verification', '${params.makeQueryString(params.paginationInfo.totalPageCount)}');" class="btn_paging last">
            ...
            <div class="result_detail">
                <table summary="분류, paper로 구성">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td id="msg_detail_context" colspan="2" style="white-space:pre-line;" class="lyr_con">
...
function searchMsg(){
	...
	var queryString = "?currentPageNo=" + $('#currentPageNo').val()
			...
			+ "&searchFit=" + $('#searchFit').val()// 일치여부 → 통과여부
...
function msgOpen(id){
    ...
    var context = $('#msgContext_'+ id).val();
        context = context.replace(/&/g, '&amp;')
                         .replace(/>/g, '&gt;')
                         .replace(/</g, '&lt;');
	//var context = $('#msgContext_'+ id).val().replace(/(?:\r\n|\r|\n)/g, '<br />');
    ...
    // 2023.02.22 문자를 숫자로 변경
    startIndex = isNaN(parseInt(startIndex, 10)) ? 0 : parseInt(startIndex, 10);
    endIndex = isNaN(parseInt(endIndex, 10)) ? 0 : parseInt(endIndex, 10);
	...
	if (isMatch=='0') {
		...
		title = '통과'; //2023-01-31 일치 → 통과로 수정
		$('#msg_detail_title').removeClass('on');
	} else {
	}
	  AS-IS end */

      if (resultLabel.indexOf('미심의') >= 0) {
      title = '불일치 광고성 문자';
      $('#msg_detail_title').addClass('on');
   } else {
      title = '일치';
    ...
    // DB에서 데이터를 가져오는 것이 아니므로 주석처리 -->  if (resultLabel.indexOf('미심의') >= 0){
    if (resultLabel.indexOf('미심의') >= 0) {
	// 텍스트  하이라이팅 추가 
	var output = '';
    ...
    //2023-02-14 '불일치 광고성 문자'인 경우만 하일라이트 추가로 변경 start
    $('#msg_detail_context').html(output);
   } else {
      $('#msg_detail_context').html(context);
   }

    //2023-02-14 '불일치 광고성 문자'인 경우만 하일라이트 추가로 변경 end
	
    /* 2023-02-14 주석 처리 start 
    $('#msg_detail_context').html(output);    
    2023-02-14 주석 처리 end */
}
...
function msgClose(){
    ...
    /* 2024-03-19(Tue) '"', '<', '>' 등 문제 처리 시작 */
    var msg = $('#msg_detail_context').val('');
		msg = msg.replace(/&/g, '&amp;')
    			 .replace(/>/g, '&gt;')
    			 .replace(/</g, '&lt;')
   				 .replace(/"/g, '&quot;');
    /* 2024-03-19(Tue) '"', '<', '>' 등 문제 처리 끝 */
}
...
function excelDown(){
    ...
    newForm.attr("action", "/user/pre_verification/excel_down");
	...
	/* 2023-01-27 내용 항목 추가 start */
    //newForm.append($('<input/>', {type: 'hidden', name: 'msg_detail_context', value: $('#h_msg_detail_context').val()}));
	/* 2023-01-27 내용 항목 추가 end */
