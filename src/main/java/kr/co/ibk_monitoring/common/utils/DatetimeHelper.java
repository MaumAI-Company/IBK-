...
public class DatetimeHelper {
	 ...
	 * @return
	 */
	public static int getInternationalAgeByBirthDate(LocalDate birthDate) {
		LocalDate now = LocalDate.now();

		int internationalAge = now.minusYears(birthDate.getYear()).getYear();

		if (birthDate.plusYears(internationalAge).isAfter(now)) {
			internationalAge = internationalAge - 1;
		}
