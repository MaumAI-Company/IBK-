...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, tvars, use_tpu=False):
  ...
  # optimizer = adamw(weight_decay=0.01, learning_rate=learning_rate, epsilon=1e-6)
  # decay_var_list = [i for i in tvars if len(set(["LayerNorm", "layer_norm", "bias"]) &
  #                                           set(i.name.replace(':', '/').replace('_', '/').split('/'))) == 0]
  ...
  train_op = optimizer.apply_gradients(
      zip(grads, tvars), global_step=global_step)#, decay_var_list=decay_var_list)
