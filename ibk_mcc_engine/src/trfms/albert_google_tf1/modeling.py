...
class AlBertConfig(object):
  ...
  @classmethod
  def from_omegaconf(cls, oc_object):
    """Constructs a `BertConfig` from a omegaconf object of parameters."""
    config = BertConfig(vocab_size=None)
    for (key, value) in oc_object.items():
      config.__dict__[key] = value
    return config
    ...
    with tf.variable_scope(scope, default_name="bert"):
      ...
      with tf.variable_scope("encoder"):
        ...
        self.all_encoder_layers = transformer_model(
            ...
            layer_sharing=config.layer_sharing)
...
def transformer_model(input_tensor,
  ...
  for layer_idx in range(num_hidden_layers):
    layer_scope = "layer_{}".format(layer_idx if not layer_sharing else 'shared')
